...
void SurfacePlot::SpecialFunc(int key, int x, int y)
{
	switch (key) {
	case GLUT_KEY_F1:
		mode = 0;
...
float SurfacePlot::function(float x)
{
	if (fun_ptr == NULL)
	{
		std::cout << "Set The function";
		assetDraw = false;
		return 0.0;
	}
	assetDraw = true;
...
void SurfacePlot::draw()
{
	
	//Border
	glColor3f(1.0f, 0.859f, .34f);
	glBindBuffer(GL_ARRAY_BUFFER, lbo);
...
void SurfacePlot::SetFunction(float(*f)(float))
{
	fun_ptr = f;
}
